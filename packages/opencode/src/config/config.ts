...
export namespace Config {
  ...
  // In-memory overlay for MCP tool/server enabled states.
  // Allows toggling without triggering Instance.dispose().
  const toolsOverlayByDir = new Map<string, Record<string, boolean>>()

  export function getToolsOverlay(dir: string): Record<string, boolean> {
    return toolsOverlayByDir.get(dir) ?? {}
  }

  export function setToolsOverlay(dir: string, overlay: Record<string, boolean>): void
  export function setToolsOverlay(dir: string, tool: string, enabled: boolean): void
  export function setToolsOverlay(dir: string, input: Record<string, boolean> | string, enabled?: boolean) {
    if (typeof input === "string") {
      const prev = toolsOverlayByDir.get(dir) ?? {}
      toolsOverlayByDir.set(dir, { ...prev, [input]: enabled ?? false })
      return
    }
    toolsOverlayByDir.set(dir, input)
  }

  export function clearToolsOverlay(dir: string) {
    ...
    Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        const patchProjectField = Effect.fn("Config.patchProjectField")(function* (field: string[], value: unknown) {
          const files = ["opencode.json", "opencode.jsonc"].map((item) => path.join(Instance.directory, item))
          const file = files.find((item) => existsSync(item)) ?? files[0]
  ...
  export async function patchProjectField(field: string[], value: unknown) {
    return runPromise((svc) => svc.patchProjectField(field, value))
  }
