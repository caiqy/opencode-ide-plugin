...
const skillPermissionOverlayByDir = new Map<string, Record<string, ConfigPermission.Action>>()

function toolPermissionOverlay(tools: Record<string, boolean>) {
  return Object.fromEntries(
    Object.entries(tools).map(([tool, enabled]) => [
      tool === "write" || tool === "edit" || tool === "patch" ? "edit" : tool,
      enabled ? "allow" : "deny",
    ]),
...
function configWithOverlays(config: Info, directory: string): Info {
  ...
  return {
    ...
    permission: withSkills,
  }
}

export function getToolsOverlay(directory: string) {
  return toolsOverlayByDir.get(directory) ?? {}
}

export function setToolsOverlay(directory: string, toolID: string, enabled: boolean) {
  toolsOverlayByDir.set(directory, {
    ...getToolsOverlay(directory),
    [toolID]: enabled,
  })
}
...
export function getSkillPermissionOverlay(directory: string) {
  return skillPermissionOverlayByDir.get(directory) ?? {}
}

export function setSkillPermissionOverlay(directory: string, skill: string, action: ConfigPermission.Action) {
  skillPermissionOverlayByDir.set(directory, {
    ...getSkillPermissionOverlay(directory),
    [skill]: action,
  })
}

export function clearSkillPermissionOverlay(directory: string) {
  skillPermissionOverlayByDir.delete(directory)
}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        for (const dir of directories) {
          ...
          if (process.env["OPENCODE_DISABLE_CONFIG_DEPENDENCY_INSTALL"] !== "true") {
    ...
    const get = Effect.fn("Config.get")(function* () {
      ...
      return yield* InstanceState.use(state, (s) => configWithOverlays(s.config, directory))
    })
    ...
    const projectConfigFile = Effect.fn("Config.projectConfigFile")(function* () {
      const dir = yield* InstanceState.directory
      const candidates = ["opencode.json", "opencode.jsonc", "config.json"].map((file) => path.join(dir, file))
      for (const file of candidates) {
    ...
    const patchProjectField = Effect.fn("Config.patchProjectField")(function* (field: string[], value: unknown) {
      const file = yield* projectConfigFile()
      const before = (yield* readConfigFile(file)) ?? "{}"
      const updated = patchJsonc(before, value, field)
