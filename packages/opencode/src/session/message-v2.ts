...
function providerMeta(metadata: Record<string, any> | undefined) {
  ...
  const { providerExecuted: _, source: _source, ...rest } = metadata
      ...
      for (const part of msg.parts) {
        ...
        if (part.type === "tool") {
          ...
          if (part.state.status === "completed") {
            const strippedMediaPaths =
              part.state.time.compacted || !options?.stripMedia
                ? []
                : (part.state.attachments ?? []).flatMap((attachment) =>
                    attachment.relativePath?.startsWith(".opencode/generated-images/") && isMedia(attachment.mime)
                      ? [`已生成图片文件：${attachment.relativePath}`]
                      : [],
                  )
            const outputText = [
