...
export function mergeToolMetadataState(
  state: Extract<SessionV1.ToolPart["state"], { status: "pending" | "running" }>,
  update: { title?: string; metadata?: Record<string, unknown> },
  args: Record<string, unknown>,
  now: number,
): Extract<SessionV1.ToolPart["state"], { status: "running" }> {
  return {
    title: update.title,
    metadata: update.metadata,
    status: "running",
    input: args,
    time: { ...(state.status === "running" ? state.time : {}), start: state.status === "running" ? state.time.start : now },
  }
}
...
export const resolve = Effect.fn("SessionTools.resolve")(function* (input: {
  ...
  const context = (args: Record<string, unknown>, options: ToolExecutionOptions, tool: string): Tool.Context => ({
    ...
    metadata: (val) =>
      input.processor.updateToolCall(options.toolCallId, (match) => {
        ...
        return {
          ...match,
          state: mergeToolMetadataState(match.state, val, args, Date.now()),
        }
      }),
    ask: (req) =>
      permission
