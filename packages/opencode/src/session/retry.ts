...
type RetryableSignal = Retryable | "stream_timeout"
...
export function retryable(error: Err, provider: string) {
  ...
  const msg =
    isRecord(error) && isRecord(error.data) && typeof error.data.message === "string" ? error.data.message : undefined
  if (typeof msg === "string") {
    ...
    if (matchesRetryableMessage(msg)) return { message: msg }
  }
  return undefined
}
