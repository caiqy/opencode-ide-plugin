...
export function recentConnectedWorkspaces<WorkspaceInfo extends { id: string; name: string; type: string; timeUsed?: number | string }>(input: {
  ...
  const workspaces = allWorkspaces.toSorted((a, b) => Number(b.timeUsed ?? 0) - Number(a.timeUsed ?? 0))
...
export async function confirmWorkspaceFileChanges(input: {
  ...
  const status = await input.sdk.client.file.status({ workspace: input.sourceWorkspaceID }).catch(() => undefined)
