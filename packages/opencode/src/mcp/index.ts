...
export namespace MCP {
  ...
  const canon = (client: string, tool: string) => sanitize(client) + "_" + sanitize(tool)
  ...
  export interface Interface {
    ...
    readonly toolsByServer: (
      name: string,
    ) => Effect.Effect<{ connected: boolean; tools: Array<{ id: string; name: string }> }>
    readonly getPrompt: (
      clientName: string,
      name: string,
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const setEnabled = Effect.fn("MCP.setEnabled")(function* (name: string, enabled: boolean) {
        ...
        yield* Effect.promise(() => Config.patchProjectField(["mcp", name, "enabled"], enabled))
      ...
      const setToolEnabled = Effect.fn("MCP.setToolEnabled")(function* (toolId: string, enabled: boolean) {
        ...
        yield* Effect.promise(() => Config.patchProjectField(["tools", toolId], enabled))

        const s = yield* InstanceState.get(cache)
        const name = Object.entries(s.defs).find(([key, listed]) => listed.some((tool) => canon(key, tool.name) === toolId))?.[0]
        if (!name) return

        s.defs[name] = [...s.defs[name]]
  ...
  export const setEnabled = async (name: string, enabled: boolean) => runPromise((svc) => svc.setEnabled(name, enabled))

  export const setToolEnabled = async (toolId: string, enabled: boolean) =>
    runPromise((svc) => svc.setToolEnabled(toolId, enabled))

  export const toolsByServer = async (name: string) => runPromise((svc) => svc.toolsByServer(name))
