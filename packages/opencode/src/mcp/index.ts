...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const setEnabled = Effect.fn("MCP.setEnabled")(function* (name: string, enabled: boolean) {
      yield* cfgSvc.patchProjectField(["mcp", name, "enabled"], enabled)
      if (!enabled) {
      ...
      const mcp = yield* getMcpConfig(name)
      if (mcp) yield* createAndStore(name, { ...mcp, enabled: true })
      yield* bus.publish(ToolsChanged, { server: name }).pipe(Effect.ignore)
    })

    const setToolEnabled = Effect.fn("MCP.setToolEnabled")(function* (toolID: string, enabled: boolean) {
      const directory = yield* InstanceState.directory
      Config.setToolsOverlay(directory, toolID, enabled)
      yield* cfgSvc.patchProjectField(["tools", toolID], enabled)
      yield* bus.publish(ToolsChanged, { server: toolID.split("_")[0] ?? toolID }).pipe(Effect.ignore)
    })
    ...
    const toolsByServer = Effect.fn("MCP.toolsByServer")(function* (name: string) {
      const s = yield* InstanceState.get(state)
      const connected = s.status[name]?.status === "connected"
      if (!connected) return { connected, tools: [] }
      const cfg = yield* cfgSvc.get()
      return {
        connected,
        tools: (s.defs[name] ?? []).map((tool) => {
          const id = sanitize(name) + "_" + sanitize(tool.name)
          return {
            id,
            name: tool.name,
            description: tool.description,
            enabled: cfg.tools?.[id] !== false,
          }
        }),
      }
    })
...
export function toolsByServer(name: string) {
  return runtime.runPromise((svc) => svc.toolsByServer(name))
}

export function setEnabled(name: string, enabled: boolean) {
  return runtime.runPromise((svc) => svc.setEnabled(name, enabled))
}

export function setToolEnabled(toolID: string, enabled: boolean) {
  return runtime.runPromise((svc) => svc.setToolEnabled(toolID, enabled))
}
