...
function create(opts: { cors?: string[] }) {
  ...
  return {
    app: app
      ...
      // WebGUI routes must be mounted before workspace routing so IDE webviews
      // can load the downstream React app from /app without being intercepted.
      .get("/app", async (c) => {
        const response = serveWebGuiPath("")
        if (!response) return c.notFound()
        return response
      })
      .route("/app", webguiGeneratedImageApp)
      .get("/app/*", async (c) => {
        const requested = c.req.path.replace(/^\/app\/?/, "")
        const response = serveWebGuiPath(requested)
        if (!response) return c.notFound()
        return response
      })
      .route("/", workspaceApp)
