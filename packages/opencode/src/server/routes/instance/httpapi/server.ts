  ...
  return Effect.gen(function* () {
    ...
    return yield* Effect.promise(async () =>
      await Instance.provide({
        directory: url.searchParams.get("directory") || request.headers["x-opencode-directory"] || process.cwd(),
        fn: async () => await readGeneratedImage(pathname),
      }),
    )
  })
}
...
const generatedImageRoute = HttpRouter.use((router) =>
  Effect.gen(function* () {
    yield* router.add("GET", "/generated-image", generatedImageResponse)
    yield* router.add("GET", "/app/generated-image", generatedImageResponse)
  }),
