...
export {
  Service,
  type CancelOwnedInput,
  type ExtendInput,
  type ExtendIfOpenInput,
  type Info,
  type Interface,
  type Promotion,
  type StartInput,
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return CoreBackgroundJob.Service.of({
      ...
      startOwned: (input) => InstanceState.useEffect(state, (jobs) => jobs.startOwned(input)),
      startOwnedIfOpen: (input) => InstanceState.useEffect(state, (jobs) => jobs.startOwnedIfOpen(input)),
      admissionsOpen: (keys) => InstanceState.useEffect(state, (jobs) => jobs.admissionsOpen(keys)),
      closeAdmissions: (keys) => InstanceState.useEffect(state, (jobs) => jobs.closeAdmissions(keys)),
      openAdmissions: (keys) => InstanceState.useEffect(state, (jobs) => jobs.openAdmissions(keys)),
      cancelOwned: (input) => InstanceState.useEffect(state, (jobs) => jobs.cancelOwned(input)),
      extend: (input) => InstanceState.useEffect(state, (jobs) => jobs.extend(input)),
      extendIfOpen: (input) => InstanceState.useEffect(state, (jobs) => jobs.extendIfOpen(input)),
