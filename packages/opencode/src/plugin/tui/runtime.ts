...
async function installPluginBySpec(
  ...
  const paths = state.api.state.path
  if (!paths.directory) {
    return {
      ok: false,
  ...
  const project = await state.api.client.project.current().then((result) => result.data)
  ...
  if (tui) {
    ...
    state.pending.set(spec, {
      ...
      source: (file ?? paths.config) || path.join(patch.dir, "tui.json"),
    })
  }
