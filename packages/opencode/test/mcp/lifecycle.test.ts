...
it.instance(
  "toolsByServer exposes sanitized ids that round-trip through setToolEnabled",
  () =>
    MCP.Service.use((mcp: MCPNS.Interface) =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        lastCreatedClientName = "alpha"
        const serverState = getOrCreateClientState("alpha")
        serverState.tools = [
          { name: "read.file", description: "reads a file", inputSchema: { type: "object", properties: {} } },
        ]
        ...
        const listed = yield* mcp.toolsByServer("alpha")
        ...
        yield* mcp.setToolEnabled(listed.tools[0]!.id, false)

        const refreshed = yield* mcp.toolsByServer("alpha")
        ...
        expect(yield* Effect.promise(() => Bun.file(path.join(test.directory, "opencode.json")).json())).toMatchObject({
          tools: {
            alpha_read_file: false,
          },
        })
      }),
    ),
