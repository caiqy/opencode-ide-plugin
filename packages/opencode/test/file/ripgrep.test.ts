...
import { Ripgrep, selectBinary } from "../../src/file/ripgrep"
...
describe("file.ripgrep", () => {
  it.live("selects a native rg instead of a node_modules bin shim", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdir((dir) =>
        Effect.gen(function* () {
          yield* mkdir(path.join(dir, "node_modules", ".bin"))
          yield* mkdir(path.join(dir, "native"))
        }),
      )
      const shim = path.join(dir, "node_modules", ".bin", process.platform === "win32" ? "rg.exe" : "rg")
      const native = path.join(dir, "native", process.platform === "win32" ? "rg.exe" : "rg")

      expect(selectBinary([shim, native])).toBe(native)
    }),
  )
