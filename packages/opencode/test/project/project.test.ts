...
function runLive<A>(effect: Effect.Effect<A, never, Project.Service>) {
...
describe("Project.fromDirectory", () => {
  ...
  test("returns a stable non-git project id for plain directories", async () => {
    ...
    const { project: a, sandbox: sandboxA } = await runLive(run((svc) => svc.fromDirectory(tmp.path)))
    const { project: b, sandbox: sandboxB } = await runLive(run((svc) => svc.fromDirectory(tmp.path)))
  ...
  test("assigns different non-git project ids to different directories", async () => {
    ...
    const { project: a } = await runLive(run((svc) => svc.fromDirectory(first.path)))
    const { project: b } = await runLive(run((svc) => svc.fromDirectory(second.path)))
  ...
  test("migrates legacy global sessions for plain directories at runtime", async () => {
    ...
    const { project: first } = await runLive(run((svc) => svc.fromDirectory(tmp.path)))
    ...
    const { project: second } = await runLive(run((svc) => svc.fromDirectory(tmp.path)))
...
describe("Project.list and Project.get", () => {
  ...
  test("list drops legacy global project rows once their non-git sessions are migrated", async () => {
    ...
    await runLive(run((svc) => svc.fromDirectory(tmp.path)))
