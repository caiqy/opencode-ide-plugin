...
export function resolvePtyConnectTicket(result?: { response: { status: number }; data?: unknown }) {
  if (!result) return
  if (result.response.status === 200) {
    // SDK/OpenAPI currently models this 200 payload as boolean, so keep a runtime ticket guard here.
    const data = result.data as unknown
    if (typeof data === "object" && data !== null && "ticket" in data && typeof data.ticket === "string") {
      return data.ticket
    }
  ...
  onMount(() => {
    const run = async () => {
      ...
      const connectToken = async () => {
        ...
        return resolvePtyConnectTicket(result)
      }
