...
import { compareMessages, normalizeSessionMessages, searchMessages } from "@/utils/session-message"
...
function mergeOptimisticPage(page: MessagePage, items: OptimisticItem[]) {
  ...
  for (const item of items) {
    const result = searchMessages(session, item.message)
  ...
  const apply = (event: { type: string; properties?: unknown }) => {
    ...
    switch (event.type) {
      ...
      case "message.updated": {
        ...
        const result = searchMessages(messages, info)
