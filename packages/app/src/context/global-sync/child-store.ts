...
import { directoryKey, emptyPath, type DirectoryKey } from "./utils"
        ...
        createRoot((dispose) => {
          ...
          const child = createStore<State>({
            ...
            get provider() {
              ...
              const globalProvider = input.global.provider()
              if (providerQuery.data?.all.length === 0 && globalProvider.all.length > 0) return globalProvider
              return providerQuery.data ?? EMPTY
            },
            config: {},
            get path() {
              if (pathQuery.isLoading || !pathQuery.data) return emptyPath()
