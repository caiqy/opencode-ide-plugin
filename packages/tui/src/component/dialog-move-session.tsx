...
import { DialogWorkspaceFileChanges, fromVcsFileStatus } from "./dialog-workspace-file-changes"
  ...
  async function remove(option: DialogSelectOption<MoveSessionSelection | undefined>) {
    ...
    if (result.error) {
      ...
      if ("data" in result.error && result.error.data.forceRequired) {
        ...
        const choice = await DialogWorkspaceFileChanges.show(dialog, (status?.data ?? []).map(fromVcsFileStatus), {
