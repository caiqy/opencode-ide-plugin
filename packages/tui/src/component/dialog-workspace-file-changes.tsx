...
import type { File as WorkspaceFileChange, VcsFileStatus } from "@opencode-ai/sdk/v2"
...
export function fromVcsFileStatus(file: VcsFileStatus): WorkspaceFileChange {
  return { path: file.file, added: file.additions, removed: file.deletions, status: file.status }
}

function statusLabel(status: WorkspaceFileChange["status"]) {
  if (status === "added") return "A"
  if (status === "deleted") return "D"
  return "M"
}

function changeCountWidth(file: WorkspaceFileChange) {
  // The "plus 2" is for spaces
  return `${file.added ? `+${file.added}` : ""}${file.removed ? ` -${file.removed}` : ""}`.length + 2
}
            ...
            <box flexDirection="row" justifyContent="space-between" paddingLeft={2} paddingRight={2}>
              <box flexDirection="row" minWidth={0} flexShrink={1}>
                ...
                <text fg={theme.textMuted} wrapMode="none">
                  {Locale.truncateLeft(item.path, fileNameWidth())}
                </text>
              </box>
              <box flexDirection="row" gap={1} minWidth={7} flexShrink={0} justifyContent="flex-end">
                <text>
                  {" "}
                  {item.added ? <span style={{ fg: theme.diffAdded }}>+{item.added}</span> : null}
                  {item.removed ? <span style={{ fg: theme.diffRemoved }}> -{item.removed}</span> : null}
                </text>
              </box>
            </box>
          )}
