...
export interface Interface {
  ...
  readonly which: (pkg: string, bin?: string, options?: PackageOptions) => Effect.Effect<string | undefined>
}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const directory = (pkg: string, registry?: string) => {
      const normalized = registry?.endsWith("/") ? registry.slice(0, -1) : registry
      const key =
        normalized && normalized !== DEFAULT_REGISTRY
          ? `${sanitize(pkg)}-${encodeURIComponent(normalized)}`
          : sanitize(pkg)
      return path.join(global.cache, "packages", key)
    }
    const reify = (input: { dir: string; add?: string[]; registry?: string }) =>
      Effect.gen(function* () {
        ...
        const npmOptions = yield* NpmConfig.load(input.dir).pipe(
          Effect.map((options) => (input.registry ? { ...options, registry: input.registry } : options)),
        )
    ...
    const add = Effect.fn("Npm.add")(function* (pkg: string, options?: PackageOptions) {
      const dir = directory(pkg, options?.registry)
      const name = (() => {
        try {
      ...
      const tree = yield* reify({ dir, add: [pkg], registry: options?.registry })
    ...
    const which = Effect.fn("Npm.which")(function* (pkg: string, bin?: string, options?: PackageOptions) {
      const dir = directory(pkg, options?.registry)
      ...
      const pick = Effect.fnUntraced(function* () {
        ...
        if (Option.isSome(pkgJson)) {
          ...
          if (parsed?.bin) {
            ...
            return parsedBin[unscoped] ? Option.some(unscoped) : Option.none<string>()
          }
        }
