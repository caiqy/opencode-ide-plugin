...
WebGUI 仅在通过受支持且当前已连接的桌面 IDE bridge 运行时，将现有 Agent 完成和权限请求事件映射为 IDE 原生通知。通知是瞬时事件，只能在事件发生时立即投递，不得改变会话、消息或权限状态。
...
8. WebGUI 仅在 `ideBridge.isInstalled()` 为 true 且 `ideBridge.ready === true` 时调用 `ideBridge.send()`。未 ready 的通知不得进入普通 bridge 命令的重连队列，bridge 后来 ready 时不得迟到展示。
...
4. bridge 已安装但未 ready 时，完成和权限通知均返回未发送，且不调用 `ideBridge.send()`。
5. VS Code handler 收到 trim 后的有效 title/body；JetBrains 拒绝缺失、空白或非字符串字段。
