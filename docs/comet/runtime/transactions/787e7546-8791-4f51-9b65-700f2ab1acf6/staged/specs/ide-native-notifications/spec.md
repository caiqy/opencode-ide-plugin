...
WebGUI 仅在通过受支持且当前已连接的桌面 IDE bridge 运行时，将现有 Agent 完成和权限请求事件映射为 IDE 原生通知。通知是瞬时事件，只能在事件发生时尝试一次即时投递，不得改变会话、消息或权限状态。
...
4. `permission.asked` request ID 在首次观察事件时立即登记；即使该事件因当前前台聚焦会话、bridge 未安装、bridge 未 ready、不支持或未实际发送而被抑制，同一 ID 的后续重放也不得通知。`permission.replied` 后释放该 ID。
5. session error 和其他事件本身不得触发通知。
6. 当目标是当前会话，且页面可见并拥有焦点时抑制通知；其他会话仍可通知。
7. IDE bridge 未安装、当前未 ready 或 host 不支持时安全无操作。普通网页版不得使用 Web Notification 作为回退。
8. WebGUI 通知必须调用 bridge 的易失发送入口，由该入口同步完成配置和 ready 的最终门禁。未 ready 的通知不得进入普通 bridge 命令的重连队列。
9. 易失发送接受后只尝试一次 POST。网络异常或 HTTP 5xx 不得重试、延迟重试或入队；后续 SSE 断线和重连不得补发该通知。
10. 普通 `send()`、`request()`、队列和失败重试语义保持不变。
...
2. 权限 request ID 首次在前台聚焦或 bridge 未 ready 时被抑制，失焦或 bridge 后来 ready 后重放相同 ID 仍不通知；若期间已收到 `permission.replied`，也不得迟到发送原通知。
3. idle 判定只接受 `busy|retry -> idle`，拒绝所有其他前态。
4. bridge 未配置或未 ready 时，完成和权限通知均返回未发送，且不调用 fetch。
5. bridge ready 时通过 `sendIdeNotification()` 触发的 POST 若 reject 或返回 5xx，随后断线、重连和推进重试定时器后 fetch 仍仅调用一次。
6. 普通 bridge 请求在既有可重试失败下仍按原语义重试。
7. VS Code handler 收到 trim 后的有效 title/body；JetBrains 拒绝缺失、空白或非字符串字段。
