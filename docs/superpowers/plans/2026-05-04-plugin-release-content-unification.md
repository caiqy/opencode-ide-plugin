...
beforeEach(async () => {
  ...
  await writeFile(
    path.join(tempRoot, "docs/release-content/manifest.json"),
...
describe("syncReleaseContent", () => {
  it("writes generated files for both platforms", async () => {
    ...
    const vscodePackage = JSON.parse(
      await readFile(path.join(tempRoot, "hosts/vscode-plugin/package.json"), "utf8"),
    )
    const jetbrainsDescription = await readFile(
      path.join(tempRoot, "hosts/jetbrains-plugin/description.html"),
      "utf8",
    )
    const jetbrainsChangelog = await readFile(
      path.join(tempRoot, "hosts/jetbrains-plugin/changelog.html"),
      "utf8",
    )
