...
| VSCode package            | VSIX 缺少版本更新、backend binary 或 valid manifest                                    | VSCode                                   | 按 `.opencode/command/build-vsix.md` 执行 packaging validation；确认 VSIX 存在且包含 Windows amd64 binary。                                                   |
