...
| 功能                    | 原因                                                               |
| ----------------------- | ------------------------------------------------------------------ |
| 自动合并解决            | any 类型 (434+) 导致自动解决无法被类型系统验证，必须人工审查       |
| Cherry-pick 工作流      | 创建平行历史，每次 cherry-pick 都是合并债务，长期更难维护          |
| Rebase 到上游           | 384 个下游 commit 的 rebase 会重写历史，破坏 PR                    |
...
| 需求      | 阶段             | 状态 |
| --------- | ---------------- | ---- |
| SYNC-01   | 阶段 1：合并基础 | 待定 |
| SYNC-02   | 阶段 3：冲突检测 | 待定 |
| SYNC-03   | 阶段 3：冲突检测 | 待定 |
| SYNC-04   | 阶段 3：冲突检测 | 待定 |
| SYNC-05   | 阶段 1：合并基础 | 待定 |
| SYNC-06   | 阶段 1：合并基础 | 待定 |
| BUILD-01  | 阶段 2：构建验证 | 待定 |
| BUILD-02  | 阶段 2：构建验证 | 待定 |
| BUILD-03  | 阶段 2：构建验证 | 待定 |
| BUILD-04  | 阶段 2：构建验证 | 待定 |
| BUILD-05  | 阶段 2：构建验证 | 待定 |
| BUILD-06  | 阶段 2：构建验证 | 待定 |
| IMPACT-01 | 阶段 4：影响分析 | 待定 |
| IMPACT-02 | 阶段 4：影响分析 | 待定 |
| IMPACT-03 | 阶段 4：影响分析 | 待定 |
| IMPACT-04 | 阶段 4：影响分析 | 待定 |
