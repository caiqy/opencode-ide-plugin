...
void help(const std::wstring &error)
{
    if (!error.empty()) {
        ...
        std::wcerr << L"Welcome to SnoreToast " << SnoreToasts::version() << L"." << std::endl
    ...
    std::wcerr
            ...
            << std::endl
            << L"[-pid] <pid> | Target process for foreground activation; query its app id only when -appID is omitted."
            << std::endl
            << L"[-protocol] <URI>\t| Launch using a protocol URI instead of callback activation arguments."
            << std::endl
